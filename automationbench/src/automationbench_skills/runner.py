...
async def run_one_async(
    ...
    *,
    ...
    ``client`` overrides the client :func:`get_client` would build for ``model``
    (an optimizer harness supplying an instrumented one); its lifetime is the
    caller's, and ``model.name`` still selects the model requested per turn.
    """
